...
const Header = () => {
  ...
  return (
    <nav className="bg-gradient-to-r from-[#0a122a] via-[#0f1e3d] to-[#0a122a] text-gray-200 grid grid-cols-5 h-16 px-6 shadow-lg relative">
      {/* Logo */}
      ...
      {/* Hamburger Icon (Hidden when in /chat route) */}
      {location.pathname !== "/chat" && (
        <div
          id="toggleHamburger"
      ...
      {isOpen && location.pathname !== "/chat" && (
        <div
          ref={menuRef}
          className="absolute top-16 left-0 w-full bg-[#0a122a] text-gray-200 shadow-lg flex flex-col items-center space-y-4 py-4 md:hidden"
        >
          <Link to="/about" className="hover:text-blue-400 transition">
            About Us
          </Link>
